/**
 * search_eu_implementations — Search EU directives/regulations referenced by Czech legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_czech_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as czech_statute_count
  ...
  if (input.has_czech_implementation) {
    sql += ' HAVING czech_statute_count > 0';
  }
