/**
 * get_czech_implementations — Find Czech statutes that reference a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetCzechImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface CzechImplementationResult {
  document_id: string;
...
export async function getCzechImplementations(
  db: InstanceType<typeof Database>,
  input: GetCzechImplementationsInput,
): Promise<ToolResponse<CzechImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as CzechImplementationResult[];
