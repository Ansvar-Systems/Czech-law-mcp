#!/usr/bin/env tsx
/**
 * Drift detection for Czech Law MCP.
 *
 * Checks if upstream www.zakonyprolidi.cz content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Czech Law MCP — Drift Detection');
  console.log('=====================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'Czech-Law-MCP/1.0 drift-detect' },
      });
