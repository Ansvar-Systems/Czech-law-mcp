#!/usr/bin/env tsx
/**
 * Czech Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://www.zakonyprolidi.cz';
const PORTAL_NAME = 'Zakony pro lidi';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/czech-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Czech Law MCP — Data Freshness Check');
